...
class ReturnOrderItemListTVC: UITableViewCell , ShimmeringViewProtocol{
    
    static let cellIdentifierReturnOrderItemListTVC = String(describing: ReturnOrderItemListTVC.self)
    ...
    @IBOutlet var btnFullReturn : UIButton!
    @IBOutlet var btnPartialReturn : UIButton!
...
//    func reloadData(){
//        self.tableViewMultiItemContraint.constant = CGFloat(138 * 2)
//    }
    
    func reloadData(diamonds : [MyOrderDiamond]){
        self.diamondsInfo = diamonds
        
        selectedState = Array(repeating: false, count: diamonds.count)
...
//        }
    ...
    @IBAction func btnactionsReturnType(_ sender : UIButton){
        ...
        if sender.tag == 0{
            cancelletionTag = 0
            self.btnFullReturn.setImage(UIImage(named: "radioButtonSelected"), for: .normal)
            self.btnPartialReturn.setImage(UIImage(named: "radioButtonDeselected"), for: .normal)
           // self.btnActionsCancel(selectedItemsArray)
        }
        else if sender.tag == 1{
            cancelletionTag = 1
            self.btnFullReturn.setImage(UIImage(named: "radioButtonDeselected"), for: .normal)
            self.btnPartialReturn.setImage(UIImage(named: "radioButtonSelected"), for: .normal)
...
extension ReturnOrderItemListTVC : UITableViewDelegate, UITableViewDataSource {
