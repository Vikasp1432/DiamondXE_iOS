...
class KYCDocResubmittedTVC: UITableViewCell, UITextFieldDelegate {
    
    static let cellIdentifierKYCDocResubmittedTVC = String(describing: KYCDocResubmittedTVC.self)
...
//        
    ...
    @IBAction func btnActionVerify(_ sender: UIButton){
        ...
        case 0:
            self.objectVC.view.endEditing(true)
            var param :[String:Any] = ["documentType":"GSTNo", "GSTNo":self.txtGSTNum.text ?? ""]
            CustomActivityIndicator2.shared.show(in: self.objectVC.view, gifName: "diamond_logo", topMargin: 300)
    ...
    @IBAction func btnActionBrows(_ sender: UIButton){
        switch sender.tag {
        case 0:
            ImagePickerManager().pickImage(objectVC){ image in
                self.docGST = image
